...
# ==============================
# STREAMLIT APP – TV SHOW ANALYTICS
# ==============================
...
@st.cache_data
def load_data():
    df = pd.read_csv("10k_Poplar_Tv_Shows_with_users_updated4.csv")
...
# ==============================
if section == "Overview":
    st.title("📺 TV Shows Analytics Dashboard")

    st.write("### Dataset Overview")
    st.metric("Total Shows", df.shape[0])
    st.metric("Average Vote", round(df["vote_average"].mean(), 2))
    st.metric("Average Popularity", round(df["popularity"].mean(), 2))
    ...
    sns.histplot(df['popularity'].dropna(), kde=True, ax=axs[0])
    axs[0].set_title('Distribution of Popularity')
    sns.histplot(df['vote_average'].dropna(), kde=True, ax=axs[1])
    axs[1].set_title('Distribution of Vote Average')
    sns.histplot(df['vote_count'].dropna(), kde=False, ax=axs[2])
    ...
    fig = sns.pairplot(
        df[['popularity', 'vote_average', 'vote_count']].dropna(),
        diag_kind='kde',
        corner=True
    )
    st.pyplot(fig)
...
# ==============================
    ...
    st.title("👥 User Age Analysis")

    col1, col2 = st.columns(2)

    with col1:
        fig, ax = plt.subplots()
        ax.hist(df["user_age"], bins=20)
    ...
    age_duration = df.groupby('user_age')['session_duration_min'].mean().reset_index()
...
# ==============================
    ...
    prob_like = exploded.groupby(['user_age', 'genre_names'])['likes'].mean().reset_index()
    prob_like = prob_like.rename(columns={'likes': 'prob_like'})
    pivot_prob = prob_like.pivot(index='genre_names', columns='user_age', values='prob_like').fillna(0)
    ...
    df_genre_age = df.explode("genre_names")
...
# ==============================
    ...
    st.title("🍿 Binge Watching Analysis")

    df["is_binge"] = (df["popularity"] > 50) & (df["vote_count"] > 1000)
    df["binge_prob"] = df["is_binge"].astype(int) * (1 - df["user_age"] / 100)

    binge_by_age = df.groupby("user_age")["binge_prob"].mean()
    ...
    exploded = df.explode('genre_names')
    exploded['age_group'] = pd.cut(exploded['user_age'], bins=[0,20,40,60,100], labels=['0-20','21-40','41-60','61+'])
    binge_by_group_genre = exploded.groupby(['age_group', 'genre_names'])['binge_prob'].mean().reset_index()
    pivot_binge = binge_by_group_genre.pivot(index='genre_names', columns='age_group', values='binge_prob').fillna(0)
...
# ==============================
    ...
    st.title("⏰ Time of Day Watching Patterns")
    ...
    np.random.seed(42)
    df["watch_hour"] = df["user_age"].apply(sim_hour)
...
# ==============================
    ...
    exploded = df.explode("origin_country")
    ...
    age_dist_country = exploded.groupby('origin_country')['user_age'].agg(['mean', 'median', 'std', 'count']).reset_index()
    age_dist_country = age_dist_country.sort_values('count', ascending=False)
    top_country = age_dist_country.iloc[0]['origin_country']
    fig, ax = plt.subplots(figsize=(10,6))
    exploded[exploded['origin_country'] == top_country]['user_age'].hist(bins=20, ax=ax)
    ...
    df_country = df.explode("origin_country")
    ...
    df_exp = df.explode("genre_names").explode("origin_country")
...
# ==============================
    ...
    shows_per_year = df["first_air_year"].value_counts().sort_index()
    fig, ax = plt.subplots()
    ...
    df_age_genre = df.dropna(subset=["decade", "age_group"]).explode("genre_names")
...
# ==============================
    ...
    st.title("🔍 Additional Analyses")

    st.write("### Outliers in Vote Average")
    Q1 = df['vote_average'].quantile(0.25)
    Q3 = df['vote_average'].quantile(0.75)
    IQR = Q3 - Q1
    mask = (df['vote_average'] < (Q1 - 1.5 * IQR)) | (df['vote_average'] > (Q3 + 1.5 * IQR))
    outliers = df.loc[mask, ['id', 'name', 'vote_average']]
